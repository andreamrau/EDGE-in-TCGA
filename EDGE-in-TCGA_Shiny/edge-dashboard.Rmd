...
# sqlite_file <- "dashboard_data/all_values.sqlite"
# sqldb <- dbConnect(SQLite(), dbname=sqlite_file)
# dbWriteTable(conn=sqldb, name="all_values", value=all_values, 
...
# Create sqlite database ------------------------------------------------------
# sqlite_file <- "dashboard_data/TF_values.sqlite"
# sqldb <- dbConnect(SQLite(), dbname=sqlite_file)
# dbWriteTable(conn=sqldb, name="TF_values", value=TF_values,
#                 row.names=FALSE, overwrite=TRUE,
#                append=FALSE, field.types=NULL)
# dbDisconnect(sqldb)
# sqlite_file <- "dashboard_data/mir_values.sqlite"
# sqldb <- dbConnect(SQLite(), dbname=sqlite_file)
# dbWriteTable(conn=sqldb, name="mir_values", value=mir_values,
#                 row.names=FALSE, overwrite=TRUE,
#                append=FALSE, field.types=NULL)
# dbDisconnect(sqldb)
...
Throughout the app, download buttons are provided for tables and high-resolution graphics. 

If you use any of the results or figures generated by this app in a publication or presentation, please cite our work: Rau et al. (2018) Exploring drivers of gene expression in The Cancer Genome Atlas. *Bioinformatics*, bty551, https://doi.org/10.1093/bioinformatics/bty551.
...
A [preprint](https://www.biorxiv.org/content/early/2018/05/02/227926) and recently published [article](https://academic.oup.com/bioinformatics/advance-article-abstract/doi/10.1093/bioinformatics/bty551/5047764) describing the analyses underlying the tool are available. You can find additional information about TCGA at the [Genomic Data Commons](https://gdc.cancer.gov).
...
downloadHandler(filename = function() {paste('TF+miRNA-', geneTF(), '-', cancerTF(), '.csv', sep='')},
                content = function(file) {
                  ## Gather together TF results
                    ...
                    sel <- dbGetQuery(sqldb, paste0("select * from TF_values where gene ='", geneTF(), "' and cancer ='",
                                                    cancerTF(), "'"))
                  ...
                  sel <- dbGetQuery(sqldb, paste0("select * from mir_values where gene ='", genemir(), "' and cancer ='",
                                                  cancermir(), "'"))
                  dbDisconnect(sqldb) ## Disconnect when done 
...
renderDataTable({
  sqldb <- dbConnect(SQLite(), dbname = "dashboard_data/TF_values.sqlite")
  sel <- dbGetQuery(sqldb, paste0("select * from TF_values where gene ='", geneTF(), "' and cancer ='", cancerTF(), "'"))
  dbDisconnect(sqldb) ## Disconnect when done 
#  sel <- TF_values[which(TF_values$gene == geneTF() & TF_values$cancer == cancerTF()),]
  sel_list <- setNames(split(sel, seq(nrow(sel))), sel$cancer)
  type <- "TF"
...
renderDataTable({
  sqldb <- dbConnect(SQLite(), dbname = "dashboard_data/mir_values.sqlite")
  sel <- dbGetQuery(sqldb, paste0("select * from mir_values where gene ='", genemir(), "' and cancer ='", cancermir(), "'"))
  dbDisconnect(sqldb) ## Disconnect when done 
#  sel <- mir_values[which(mir_values$gene == genemir() & mir_values$cancer == cancermir()),]
  sel_list <- setNames(split(sel, seq(nrow(sel))), sel$cancer)
  type <- "mirs"
...
The EDGE in TCGA resource is available under the GNU public license (GPL-3). If you use any of the results or figures generated by this app in a publication or presentation, please cite our work: Rau et al. (2018) Exploring drivers of gene expression in The Cancer Genome Atlas. *Bioinformatics*, bty551, https://doi.org/10.1093/bioinformatics/bty551.
